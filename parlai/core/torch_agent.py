...
class TorchAgent(ABC, Agent):
    ...
    def add_cmdline_args(
        ...
        optim_group.add_argument(
            '-ohreset',
            '--optimizer-hard-reset',
            type='bool',
            default=False,
            help='If true, try to hard-reset the optimizer values & lr scheduler '
            'for fine-tuning. Sometimes ParlAI overrides command-line learning rate '
            'and uses the value from previous run. Enable this option for that case.'
        )
    ...
    def init_optim(self, params, optim_states=None, saved_optim_type=None):
        ...
        if opt.get('optimizer_hard_reset', False):
            return
    ...
    def batchify(self, obs_batch, sort=False):
        ...
        # LABEL_CANDIDATES_SCORE
        cand_scores = None
        if any('label_candidates_scores' in ex for ex in exs):
            cand_scores = [ex.get('label_candidates_scores', None) for ex in exs]

        # LABEL_CANDIDATES_MASKS
        cand_masks = None
        if any('label_candidates_masks' in ex for ex in exs):
            cand_masks = [ex.get('label_candidates_masks', None) for ex in exs]

        # LABEL_SCORE
        label_scores = None
        if any('label_score' in ex for ex in exs):
            label_scores = [ex.get('label_score', None) for ex in exs]
